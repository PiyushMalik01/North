...
interface OverlayMenuProps {
  ...
  isOpen: boolean;
  onClose?: () => void;
  isCodeSpacesPage?: boolean;
}

export const OverlayMenu: React.FC<OverlayMenuProps> = ({ columns, bottomText, isOpen, onClose, isCodeSpacesPage = false }) => {
  if (!isOpen) return null;

  const getItemHref = (item: string) => {
    const itemLower = item.toLowerCase();
    if (itemLower === 'online ide') return '/codespaces';
  ...
  return (
    <div className={`fixed left-0 right-0 bg-[var(--background-primary)] border-t border-[var(--border-color)] shadow-2xl z-40 animate-in fade-in slide-in-from-top-2 duration-200 ${
      isCodeSpacesPage ? 'top-14' : 'top-16 lg:top-20 xl:top-24'
    }`}>
